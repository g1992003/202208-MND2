		...
		if ($value["name"] != "") { //確認是否有檔案傳入
			...
			if ($file_url != "") { //確認是否有檔案傳入
				...
				if ($sub_name == ".jpg" || $sub_name == ".jpeg" || $sub_name == ".png") {
					...
					$src_w = imagesx($src);
					$src_h = imagesy($src);
					$need_w = $_POST["w-$key"] == "" ? 10000 : $_POST["w-$key"];
					$need_h = $_POST["h-$key"] == "" ? 10000 : $_POST["h-$key"];
					if (($_POST["w-$key"] == "" && $_POST["h-$key"] == "") || ($src_w <= $_POST["w-$key"] && $src_h <= $_POST["h-$key"])) {
						goto no_change;
					} else {
				...
				} elseif ($sub_name == ".pdf" || $sub_name == ".doc" || $sub_name == ".docx" || $sub_name == ".xls" || $sub_name == ".xlsx" || $sub_name == ".csv") {
					no_change:
					move_uploaded_file($file_url, $file_dir . $value); //上傳檔案
				} else {
					echo "<script>alert('檔案格式錯誤：為避免安全性問題，圖片請上傳JPG、PNG檔，文件檔案請上傳PDF、WORD、EXCEL檔。');history.go(-1);</script>"; //執行導頁或輸出訊息後導頁
					exit();
				}
			} else {
