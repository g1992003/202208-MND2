...
if ($Authenticate == true) {
	...
	foreach ($_POST as $key => $value) { //將post過來的參數取出
		...
		if (!isset($check[1])) {
			...
			$sqlstr .= "$key = '" . filtration($value, $link) . "'"; //將參數放入字串
		}
	}
	foreach ($_FILES as $key => $value) { //將post過來的參數取出
		if ($value["name"] != "") { //確認是否有檔案傳入
			...
			if ($file_url != "") { //確認是否有檔案傳入
				...
				if ($sub_name == ".jpg" || $sub_name == ".jpeg" || $sub_name == ".png") {
				...
				} elseif ($sub_name == ".pdf" || $sub_name == ".doc" || $sub_name == ".docx" || $sub_name == ".xls" || $sub_name == ".xlsx" || $sub_name == ".csv" || $sub_name == ".zip" || $sub_name == ".rar" || $sub_name == ".mp4") {
					no_change:
					...
					echo "<script>alert('檔案格式錯誤：圖片請上傳JPG、PNG檔，文件檔案請上傳PDF、WORD、EXCEL檔，壓縮檔案請上傳ZIP、RAR檔。');history.go(-1);</script>"; //執行導頁或輸出訊息後導頁
					exit();
				}
			} else {
