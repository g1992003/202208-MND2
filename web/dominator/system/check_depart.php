...
$name_set = "dc_name";
$title_set = "dc_title";
$account_set = "dc_account";
$password_set = "dc_password";

//用來確認帳號是否正常可使用的sql
$check_query = "SELECT $id_set FROM $db_set WHERE $account_set= :username";
...
if (@$check_result->rowCount() == 1) { //確認是否只有抓到一筆帳號紀錄，如不只一筆或沒資料，則應並非以正常程序登入或帳號錯誤，所以直接錯誤。
	...
	$user_result = $link->prepare("SELECT $id_set,$name_set,$title_set FROM $db_set WHERE $account_set = :user AND $password_set = :pass", array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
	...
	if (@$user_result->rowCount() == 1) { //判斷帳號密碼是否正確
		...
		//並寫入登入紀錄
		$type_identity = "【處室管理者】" . $user_row->$name_set;
		...
		$stmt->bindParam(':r_name', $type_identity, PDO::PARAM_STR);
		...
		$_SESSION["depart_name"] = $user_row->$title_set;
		$_SESSION["dominator_main"] = 3;
		$_SESSION["dominator_sub"] = 0;
