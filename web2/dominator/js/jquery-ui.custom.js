	...
	$.fn
			.extend({
				...
				scrollParent : function() {
					var scrollParent;
					if (($.ui.ie && (/(static|relative)/).test(this
							.css("position")))
				...
				zIndex : function(zIndex) {
					...
					if (this.length) {
						...
						while (elem.length && elem[0] !== document) {
							...
							// This makes behavior of this function consistent
							// across browsers
			...
			.extend(
					$.expr[":"],
					{
						...
						focusable : function(element) {
							return focusable(element, !isNaN($.attr(element,
									"tabindex")));
						},
	...
	if (!$("<a>").outerWidth(1).jquery) {
		$.each([ "Width", "Height" ], function(i, name) {
			var side = name === "Width" ? [ "Left", "Right" ] : [ "Top",
					"Bottom" ], type = name.toLowerCase(), orig = {
			...
			.extend(
					$.ui,
					{
						...
						hasScroll : function(el, a) {
							...
							// If overflow is hidden, the element might have
							// extra content, but the user wants to hide it
							...
							// TODO: determine which cases actually cause this
							// to happen
							// if the element doesn't have the scroll set, see
							// if it's possible to
							// set the scroll
							el[scroll] = 1;
							has = (el[scroll] > 0);
		...
		constructor.prototype = $.widget
				.extend(
						basePrototype,
						{
							...
							// always use the name + a colon as the prefix,
							// e.g., draggable:start
				...
				this
						.each(function() {
							...
							if (methodValue !== instance
								...
								returnValue = methodValue && methodValue.jquery ? returnValue
			...
			.widget(
					"ui.mouse",
					{
						...
						_mouseDown : function(event) {
							...
							// event.target.nodeName works around a bug in IE 8
							// with
							// disabled inputs (#7620)
							elIsCancel = (typeof this.options.cancel === "string"
									&& event.target.nodeName ? $(event.target)
						...
						_mouseMove : function(event) {
							// IE mouseup check - mouseup happened when mouse
							// was out of window
							if ($.ui.ie
						...
						// These are placeholder methods, to be overriden by
						// extending plugin
...
(function($, undefined) {
	...
	$.position = {
		...
		getScrollInfo : function(within) {
			var overflowX = within.isWindow ? "" : within.element
					.css("overflow-x"), overflowY = within.isWindow ? ""
		...
		getWithinInfo : function(element) {
			...
			return {
				...
				width : isWindow ? withinElement.width() : withinElement
						.outerWidth(),
				height : isWindow ? withinElement.height() : withinElement
						.outerHeight()
			};
		}
	};
...
(function($, undefined) {
			...
			.widget(
					...
					$.ui.mouse,
					{
						...
						_mouseCapture : function(event) {
							...
							$(o.iframeFix === true ? "iframe" : o.iframeFix)
									.each(
											function() {
												$(
						...
						_mouseStart : function(event) {
							...
							this.offsetParentCssPosition = this.offsetParent
									.css("position");
						...
						_getHandle : function(event) {
							return this.options.handle ? !!$(event.target)
									.closest(
											this.element
						...
						_createHelper : function(event) {
							...
							var o = this.options, helper = $
									.isFunction(o.helper) ? $(o.helper.apply(
						...
						_generatePosition : function(event) {
							...
							var containment, co, top, left, o = this.options, scroll = this.cssPosition === "absolute"
									&& !(this.scrollParent[0] !== document && $
											.contains(this.scrollParent[0],
													this.offsetParent[0])) ? this.offsetParent
									: this.scrollParent, pageX = event.pageX, pageY = event.pageY;
							...
							if (this.originalPosition) {
								...
								if (o.grid) {
									...
									pageY = containment ? ((top
											...
											: ((top - this.offset.click.top >= containment[1]) ? top
													- o.grid[1]
													: top + o.grid[1]))
											: top;
									...
									pageX = containment ? ((left
											...
											: ((left - this.offset.click.left >= containment[0]) ? left
													- o.grid[0]
													: left + o.grid[0]))
											: left;
								}
						...
						_trigger : function(type, event, ui) {
							...
							return $.Widget.prototype._trigger.call(this, type,
									event, ui);
						},
			...
			.add(
					...
					"connectToSortable",
					{
						start : function(event, ui) {
							...
							var inst = $(this).data("ui-draggable"), o = inst.options, uiSortable = $
									.extend({}, ui, {
										item : inst.element
									});
							inst.sortables = [];
									...
									.each(
											function() {
												...
												if (sortable
													...
													sortable.refreshPositions(); // Call
																					// the
																					// sortable's
																					// refreshPositions
																					// at
						...
						stop : function(event, ui) {
							...
							// If we are still over the sortable, we fake the
							// stop event of the sortable, but also remove
							// helper
							var inst = $(this).data("ui-draggable"), uiSortable = $
									.extend({}, ui, {
										item : inst.element
									});
									...
									.each(
											...
											function() {
												if (this.instance.isOver) {
													...
													inst.cancelHelperRemoval = true; // Don't
																						// remove
																						// the
																						// helper
																						// in
																						...
																						// draggable
																						// instance
													this.instance.cancelHelperRemoval = false; // Remove
																								// it
																								// in
																								// the
																								// sortable
													...
													// The sortable revert is
													// supported, and we have to
													// set a temporary dropped
													// variable on the draggable
													// to support revert:
													// "valid/invalid"
													...
													// If the helper has been
													// the original item,
													// restore properties in the
													// sortable
							...
							$
									.each(
											...
											function() {
												...
												// Copy over some variables to
												// allow calling the sortable's
												...
												if (innermostIntersecting) {
													// If it intersects, we use
													// a little isOver variable
													// and set it once, so our
													// move-in stuff gets fired
													// only once
													if (!this.instance.isOver) {

														this.instance.isOver = 1;
														// Now we fake the start
														// of dragging for the
														// sortable instance,
														// by cloning the list
														// group item, appending
														// it to the sortable
														// and using it as
														// inst.currentItem
														// We can then fire the
														// start event of the
														// sortable with our
														// passed browser event,
														// and our own helper
														...
														// Because the browser
														// event is way off the
														// new appended portlet,
														// we modify a couple of
													...
													// Provided we did all the
													// previous steps, we can
													// fire the drag event of
													// the sortable on every
													// draggable drag, when it
													// intersects with the
													// sortable
													...
													// with the sortable, and it
													// intersected before,
													// we fake the drag stop of
													// the sortable, but make
													// sure it doesn't remove
													// the helper by using
													// cancelHelperRemoval
													if (this.instance.isOver) {
			...
			.add(
					...
					"snap",
					{
						...
						drag : function(event, ui) {
							...
							var ts, bs, ls, rs, l, r, t, b, i, first, inst = $(
									this).data("ui-draggable"), o = inst.options, d = o.snapTolerance, x1 = ui.offset.left, x2 = x1
							...
							for (i = inst.snapElements.length - 1; i >= 0; i--) {
								...
								inst.snapElements[i].snapping = (ts || bs || ls
										|| rs || first);
	...
	$.ui.plugin
			.add(
					...
					{
						start : function() {
							var min, o = this.data("ui-draggable").options, group = $
									.makeArray($(o.stack))
									.sort(
											function(a, b) {
...
(function($, undefined) {
	...
	$.ui.intersect = function(draggable, droppable, toleranceMode) {
		...
		var draggableLeft, draggableTop, x1 = (draggable.positionAbs || draggable.position.absolute).left, x2 = x1
				...
				+ draggable.helperProportions.height, l = droppable.offset.left, r = l
				+ droppable.proportions.width, t = droppable.offset.top, b = t
							...
							function() {
								...
								var parentInstance, scope, parent, intersects = $.ui
										.intersect(draggable, this,
												this.options.tolerance), c = !intersects
										&& this.isover ? "isout" : (intersects
								...
								if (this.options.greedy) {
									...
									parent = this.element
											...
											.filter(
													function() {
														return $.data(this,
																"ui-droppable").options.scope === scope;
													});
					...
					$.ui.mouse,
					{
						...
						_create : function() {
							...
							this.handles = o.handles
									|| (!$(".ui-resizable-handle", this.element).length ? "e,s,se"
											: {
												n : ".ui-resizable-n",
												e : ".ui-resizable-e",
												s : ".ui-resizable-s",
							...
							this._renderAxis = function(target) {
								...
								for (i in this.handles) {
									...
									if (this.elementIsWrapper
										...
										padWrapper = /sw|ne|nw|se|n|s/.test(i) ? axis
												.outerHeight()
												: axis.outerWidth();
						...
						_mouseStart : function(event) {
							...
							var curleft, curtop, cursor, o = this.options, iniPos = this.element
									.position(), el = this.element;
							...
							cursor = $(".ui-resizable-" + this.axis).css(
									"cursor");
							$("body").css(
									"cursor",
									cursor === "auto" ? this.axis + "-resize"
											: cursor);
						...
						_updateVirtualBoundaries : function(forceAspectRatio) {
							...
							b = {
								minWidth : isNumber(o.minWidth) ? o.minWidth
										: 0,
								maxWidth : isNumber(o.maxWidth) ? o.maxWidth
										: Infinity,
								minHeight : isNumber(o.minHeight) ? o.minHeight
										: 0,
						...
						_respectSize : function(data) {
							...
							var o = this._vBoundaries, a = this.axis, ismaxw = isNumber(data.width)
									&& o.maxWidth && (o.maxWidth < data.width), ismaxh = isNumber(data.height)
									&& o.maxHeight
									&& (o.maxHeight < data.height), isminw = isNumber(data.width)
									...
									+ this.size.height, cw = /sw|nw|w/.test(a), ch = /nw|ne|n/
									.test(a);
							if (isminw) {
						...
						_proportionallyResize : function() {
							...
							var i, j, borders, paddings, prel, element = this.helper
									|| this.element;
						...
						_propagate : function(n, event) {
							...
							(n !== "resize" && this._trigger(n, event, this
									.ui()));
						},
						...
						stop : function(event) {
							var that = $(this).data("ui-resizable"), o = that.options, pr = that._proportionallyResizeElements, ista = pr.length
								...
								height : (that.size.height - soffseth)
									...
									|| null, top = (parseInt(that.element
									.css("top"), 10) + (that.position.top - that.originalPosition.top))
									|| null;
					...
					"containment",
					{
						...
						start : function() {
							var element, p, co, ch, cw, width, height, that = $(
									this).data("ui-resizable"), o = that.options, el = that.element, oc = o.containment, ce = (oc instanceof $) ? oc
									.get(0)
									: (/parent/.test(oc)) ? el.parent().get(0)
											: oc;
							...
							// i'm a node, so compute top, left, right, bottom
								...
								width = ($.ui.hasScroll(ce, "left") ? ce.scrollWidth
										: cw);
								height = ($.ui.hasScroll(ce) ? ce.scrollHeight
										: ch);
						...
						resize : function(event) {
							...
							woset = Math.abs((that._helper ? that.offset.left
									- cop.left : (that.offset.left - cop.left))
									+ that.sizeDiff.width);
							hoset = Math.abs((that._helper ? that.offset.top
									- cop.top : (that.offset.top - co.top))
									+ that.sizeDiff.height);

							isParent = that.containerElement.get(0) === that.element
									.parent().get(0);
							isOffsetRelative = /relative|absolute/
									.test(that.containerElement.css("position"));
	...
	$.ui.plugin
			.add(
					...
					"alsoResize",
					{
						...
						start : function() {
							var that = $(this).data("ui-resizable"), o = that.options, _store = function(
									exp) {
								$(exp).each(function() {
									var el = $(this);
										...
										.each(
												function() {
													var el = $(this), start = $(
															this)
															.data(
																	"ui-resizable-alsoresize"), style = {}, css = c
															&& c.length ? c
															: el
					...
					"grid",
					{
						...
						resize : function() {
							var that = $(this).data("ui-resizable"), o = that.options, cs = that.size, os = that.originalSize, op = that.originalPosition, a = that.axis, grid = typeof o.grid === "number" ? [
									...
									* gridX, oy = Math
									.round((cs.height - os.height) / gridY)
					...
					$.ui.mouse,
					{
						...
						_mouseCapture : function(event, overrideHandle) {
							...
							if (this.options.handle && !overrideHandle) {
								$(this.options.handle, currentItem).find("*")
						...
						serialize : function(o) {
							...
							var items = this
									._getItemsAsjQuery(o && o.connected), str = [];
							o = o || {};
						...
						toArray : function(o) {
							...
							var items = this
									._getItemsAsjQuery(o && o.connected), ret = [];
						...
						_intersectsWithPointer : function(item) {
							...
							var isOverElementHeight = (this.options.axis === "x")
									|| isOverAxis(this.positionAbs.top
											+ this.offset.click.top, item.top,
						...
						_intersectsWithSides : function(item) {
							...
							var isOverBottomHalf = isOverAxis(
									...
									this.positionAbs.left
											+ this.offset.click.left, item.left
											+ (item.width / 2), item.width), verticalDirection = this
									._getDragVerticalDirection(), horizontalDirection = this
									._getDragHorizontalDirection();
						...
						_createPlaceholder : function(that) {
							...
							// Create the placeholder
							that.placeholder = $(o.placeholder.element.call(
									that.element, that.currentItem));
						...
						_contactContainers : function(event) {
							...
							if (this.containers.length === 1) {
								...
								for (j = this.items.length - 1; j >= 0; j--) {
									...
									nearBottom = false;
									if (Math.abs(cur - base) > Math.abs(cur
											+ this.items[j][sizeProperty]
											- base)) {
										nearBottom = true;
								...
								this.containers[innermostIndex]._trigger(
										"change", event, this._uiHash(this));
						...
						_clear : function(event, noPropagation) {
							...
							if (this.fromOutside && !noPropagation) {
								delayedTriggers.push(function(event) {
									this._trigger("receive", event, this
							...
							if (this !== this.currentContainer) {
								if (!noPropagation) {
									delayedTriggers.push(function(event) {
										this._trigger("remove", event, this
												._uiHash());
									});
									delayedTriggers.push((function(c) {
										return function(event) {
											c._trigger("receive", event, this
													._uiHash(this));
										};
									}).call(this, this.currentContainer));
									delayedTriggers.push((function(c) {
										return function(event) {
											c._trigger("update", event, this
													._uiHash(this));
										};
									}).call(this, this.currentContainer));
								}
			...
			.widget(
					"ui.accordion",
					{
						...
						_getCreateEventData : function() {
							return {
								header : this.active,
								panel : !this.active.length ? $() : this.active
										.next(),
						...
						_setupEvents : function(event) {
							...
							if (event) {
								$.each(event.split(" "), function(index,
										eventName) {
						...
						_eventHandler : function(event) {
							var options = this.options, active = this.active, clicked = $(event.currentTarget), clickedIsActive = clicked[0] === active[0], collapsing = clickedIsActive
									...
									: clicked.next(), toHide = active.next(), eventData = {
								oldHeader : active,
								oldPanel : toHide,
							...
							options.active = collapsing ? false : this.headers
									.index(clicked);
						...
						_animate : function(toShow, toHide, data) {
							var total, easing, duration, that = this, adjust = 0, down = toShow.length
									&& (!toHide.length || (toShow.index() < toHide
											.index())), animate = this.options.animate
									|| {}, options = down && animate.down
							...
							if (!toHide.length) {
								return toShow.animate(showProps, duration,
										easing, complete);
							}
							if (!toShow.length) {
								return toHide.animate(hideProps, duration,
										easing, complete);
							}
						...
						_toggleComplete : function(data) {
							...
							toHide.removeClass("ui-accordion-content-active")
									.prev().removeClass("ui-corner-top")
	...
	$
			.widget(
					"ui.autocomplete",
					{
						...
						_create : function() {
							// Some browsers only repeat keydown events, not
							// keypress events,
							// so we use the suppressKeyPress flag to determine
							// if we've already
							// handled the keydown event. #7269
							// Unfortunately the code for & in keypress is the
							// same as the up arrow,
							// so we use the suppressKeyPressRepeat flag to
							// avoid handling keypress
							// events when we know the keydown event was used to
							// modify the
							// search term. #7799
							...
							isInput ? false :
							// All other element types are determined by whether
							// or not they're contentEditable
							...
							this._on(this.element, {
								keydown : function(event) {
									...
									case keyCode.ESCAPE:
										if (this.menu.element.is(":visible")) {
											...
											// Single press can mean undo or
											// clear
											// Double press in IE means clear
											// the whole form
											event.preventDefault();
										}
										break;
								...
								keypress : function(event) {
									...
									// replicate some key handlers to allow them
									// to repeat in Firefox and Opera
									var keyCode = $.ui.keyCode;
									switch (event.keyCode) {
									case keyCode.PAGE_UP:
										this._move("previousPage", event);
							...
							this.menu = $("<ul>").addClass(
									"ui-autocomplete ui-front").appendTo(
									this._appendTo()).menu({
									...
									._on(
											this.menu.element,
											{
												mousedown : function(event) {
													// prevent moving focus out
													// of the text field
													event.preventDefault();

													// IE doesn't prevent moving
													// focus even with
													// event.preventDefault()
													// so we set a flag to know
													// when we should ignore the
													// blur event
													...
													// causes focus to shift to
													// the body
													// but we can't detect a
													// mouseup or a click
													// immediately afterward
													// so we have to track the
													// next mousedown and close
													// the menu if
												...
												menuselect : function(event, ui) {
													...
													// only trigger when focus
													// was lost (click on menu)
													if (this.element[0] !== this.document[0].activeElement) {
														this.element.focus();
														this.previous = previous;
														// #6109 - IE triggers
														// two focus events and
														// the second
														// is asynchronous, so
													...
													// reset the term after the
													// select event
													// this allows custom select
							...
							// turning off autocomplete prevents the browser
							// from remembering the
							// value when navigating through history, so we
							// re-enable autocomplete
							// if the page is unloaded before the widget is
							// destroyed. #7790
						...
						search : function(value, event) {
							...
							// always save the actual value, not the one passed
							// as an argument
							this.term = this._value();
						...
						__response : function(content) {
							...
							if (!this.options.disabled && content
									&& content.length && !this.cancelSearch) {
						...
						_normalize : function(items) {
							// assume all items have the right format when the
						...
						_renderItem : function(ul, item) {
							return $("<li>").append($("<a>").text(item.label))
									.appendTo(ul);
						},
						...
						_keyEvent : function(keyEvent, event) {
							if (!this.isMultiLine
								...
								// prevents moving cursor to beginning/end of
								// the text field in some browsers
								event.preventDefault();
							}
						}
					});
	...
	$
			.widget(
					"ui.button",
					{
						...
						_create : function() {
							this.element.closest("form").unbind(
									"reset" + this.eventNamespace).bind(
							...
							if (this.type === "checkbox") {
								...
								this.buttonElement
										...
										// see #8559, we bind to blur here in
										// case the button element loses
										// focus between keydown and keyup, it
										// would be left in an "active" state
										.bind(
								...
								if (this.buttonElement.is("a")) {
									this.buttonElement
											.keyup(function(event) {
												if (event.keyCode === $.ui.keyCode.SPACE) {
													// TODO pass through
													// original event correctly
													// (just as 2nd argument
													// doesn't work)
													$(this).click();
												}
											});
						...
						_destroy : function() {
							...
							this.buttonElement.removeClass(
									baseClasses + " " + stateClasses + " "
											+ typeClasses).removeAttr("role")
									.removeAttr("aria-pressed").html(
						...
						refresh : function() {
							// See #8237 & #8828
							var isDisabled = this.element.is("input, button") ? this.element
									.is(":disabled")
									: this.element
						...
						_resetButton : function() {
							...
							var buttonElement = this.buttonElement
									...
									.addClass("ui-button-text").html(
											this.options.label).appendTo(
											buttonElement.empty()).text(), icons = this.options.icons, multipleIcons = icons.primary
					...
					"ui.buttonset",
					{
						...
						refresh : function() {
							...
							this.buttons = this.element
									...
									.filter(":first").addClass(
											rtl ? "ui-corner-right"
													: "ui-corner-left").end()
									.filter(":last").addClass(
											rtl ? "ui-corner-left"
													: "ui-corner-right").end()
									.end();
						},
	...
	$
			.extend(
					Datepicker.prototype,
					{
						 ...
						 * object
						 */
						_dialogDatepicker : function(input, date, onSelect,
								settings, pos) {
						...
						_destroyDatepicker : function(target) {
							var nodeName, $target = $(target), inst = $.data(
									target, PROP_NAME);
						...
						_doKeyDown : function(event) {
							...
							if ($.datepicker._datepickerShowing) {
								...
								case 13:
									sel = $("td." + $.datepicker._dayOverClass
											+ ":not(."
						...
						_showDatepicker : function(input) {
							...
							beforeShowSettings = beforeShow ? beforeShow.apply(
									input, [ input, inst ]) : {};
						...
						_updateDatepicker : function(inst) {
							...
							var origyearshtml, numMonths = this
									._getNumberOfMonths(inst), cols = numMonths[1], width = 17;
							...
							if (inst.yearshtml) {
								...
								setTimeout(
										function() {
											// assure that inst.yearshtml didn't
											// change.
						...
						_checkOffset : function(inst, offset, isFixed) {
							var dpWidth = inst.dpDiv.outerWidth(), dpHeight = inst.dpDiv
									...
									.outerHeight() : 0, viewWidth = document.documentElement.clientWidth
						...
						_updateAlternate : function(inst) {
							var altFormat, date, dateStr, altField = this._get(
									inst, "altField");
						...
						iso8601Week : function(date) {
							...
							checkDate.setDate(checkDate.NOW() + 4
									- (checkDate.getDay() || 7));
						...
						parseDate : function(format, value, settings) {
							...
							getNumber = function(match) {
								var isDoubled = lookAhead(match), size = (match === "@" ? 14
										: (match === "!" ? 20 : (match === "y"
												&& isDoubled ? 4
												: (match === "o" ? 3 : 2)))), digits = new RegExp(
										"^\\d{1," + size + "}"), num = value
										.substring(iValue).match(digits);
								if (!num) {
						...
						_setDateFromField : function(inst, noDefault) {
							...
							inst.drawYear = inst.selectedYear = date
									.getFullYear();
							inst.currentDay = (dates ? date.NOW() : 0);
						...
						_getDefaultDate : function(inst) {
							return this._restrictMinMax(inst, this
									._determineDate(inst, this._get(inst,
						...
						_determineDate : function(inst, date, defaultDate) {
							var offsetNumeric = function(offset) {
								...
								var date = (offset.toLowerCase().match(/^c/) ? $.datepicker
										._getDate(inst)
										: null)
										|| new Date(), year = date
										.getFullYear(), month = date.getMonth(), day = date
										.NOW(), pattern = /([+\-]?[0-9]+)\s*(d|D|w|W|m|M|y|Y)?/g, matches = pattern
										.exec(offset);
						...
						_setDate : function(inst, date, noChange) {
							var clear = !date, origMonth = inst.selectedMonth, origYear = inst.selectedYear, newDate = this
									._restrictMinMax(inst, this._determineDate(
											inst, date, new Date()));
						...
						_attachHandlers : function(inst) {
							var stepMonths = this._get(inst, "stepMonths"), id = "#"
									+ inst.id.replace(/\\\\/g, "\\");
							inst.dpDiv
									.find("[data-handler]")
									.map(
						...
						_generateHTML : function(inst) {
							var maxDraw, prevText, prev, nextText, next, currentText, gotoDate, controls, buttonPanel, firstDay, showWeek, dayNames, dayNamesMin, monthNames, monthNamesShort, beforeShowDay, showOtherMonths, selectOtherMonths, defaultDate, html, dow, row, group, col, selectedDate, cornerClass, calender, thead, day, daysInMonth, leadDays, curRows, numRows, printDate, dRow, tbody, daySettings, otherMonth, unselectable, tempDate = new Date(), today = this
									._daylightSavingAdjust(new Date(tempDate
											.getFullYear(),
							...
							isRTL = this._get(inst, "isRTL"), showButtonPanel = this
									...
									._get(inst, "navigationAsDateFormat"), numMonths = this
							...
							prev = (this._canAdjustMonth(inst, -1, drawYear,
									drawMonth) ? "<a class='ui-datepicker-prev ui-corner-all' data-handler='prev' data-event='click'"
									+ " title='"
									+ prevText
							...
							next = (this._canAdjustMonth(inst, +1, drawYear,
									drawMonth) ? "<a class='ui-datepicker-next ui-corner-all' data-handler='next' data-event='click'"
									+ " title='"
									+ nextText
									+ "'><span class='ui-icon ui-icon-circle-triangle-"
									+ (isRTL ? "w" : "e")
							...
							monthNamesShort = this
									._get(inst, "monthNamesShort");
							beforeShowDay = this._get(inst, "beforeShowDay");
							showOtherMonths = this
									._get(inst, "showOtherMonths");
						...
						_restrictMinMax : function(inst, date) {
							var minDate = this._getMinMaxDate(inst, "min"), maxDate = this
						...
						_canAdjustMonth : function(inst, offset, curYear,
								curMonth) {
							var numMonths = this._getNumberOfMonths(inst), date = this
									._daylightSavingAdjust(new Date(
											curYear,
											curMonth
													+ (offset < 0 ? offset
	...
	function bindHover(dpDiv) {
		...
		return dpDiv
				...
				.delegate(
						...
						function() {
							if (!$.datepicker
									._isDisabledDatepicker(instActive.inline ? dpDiv
											.parent()[0]
											: instActive.input[0])) {
			...
			.widget(
					"ui.dialog",
					{
						...
						_create : function() {
							...
							this.element.show().removeAttr("title").addClass(
						...
						_destroy : function() {
							...
							this.element.removeUniqueId().removeClass(
						...
						_moveToTop : function(event, silent) {
							var moved = !!this.uiDialog.nextAll(":visible")
						...
						_createWrapper : function() {
							this.uiDialog = $("<div>").addClass(
									...
									.attr({
										// Setting tabIndex makes the div
										// focusable
										tabIndex : -1,
										role : "dialog"
						...
						_createTitlebar : function() {
							...
							uiDialogTitle = $("<span>").uniqueId().addClass(
									"ui-dialog-title").prependTo(
									...
									.draggable({
										...
										start : function(event, ui) {
											$(this).addClass(
													"ui-dialog-dragging");
											that._blockFrames();
											that._trigger("dragStart", event,
													filteredUi(ui));
										},
										drag : function(event, ui) {
											that._trigger("drag", event,
													filteredUi(ui));
										},
										stop : function(event, ui) {
											options.position = [
							...
							this.uiDialog.resizable(
									{
										...
										stop : function(event, ui) {
											...
											$(this).removeClass(
													"ui-dialog-resizing");
											that._unblockFrames();
											that._trigger("resizeStop", event,
													filteredUi(ui));
										}
						...
						_setOption : function(key, value) {
							...
							if (key === "dialogClass") {
								uiDialog.removeClass(this.options.dialogClass)
										.addClass(value);
							}
							...
							if (key === "resizable") {
								...
								if (isResizable && typeof value === "string") {
									uiDialog.resizable("option", "handles",
											value);
								}
						...
						_createOverlay : function() {
							...
							this.overlay = $("<div>").addClass(
									"ui-widget-overlay ui-front").appendTo(
									this._appendTo());
			...
			.widget(
					"ui.menu",
					{
						...
						_create : function() {
							...
							this
									._on({
										// Prevent focus from sticking to links
										// inside menu after clicking
										// them (focus should always stay on UL
										// during navigation).
										"mousedown .ui-menu-item > a" : function(
												event) {
											event.preventDefault();
										},
										"click .ui-state-disabled > a" : function(
												event) {
											event.preventDefault();
										},
										"click .ui-menu-item:has(a)" : function(
												event) {
											...
											if (!this.mouseHandled
												...
												if (target.has(".ui-menu").length) {
													this.expand(event);
														...
														.is(":focus")) {
													// Redirect focus to the
													// menu
													this.element.trigger(
															"focus", [ true ]);

													// If the active item is on
													// the top level, let it
													// stay active.
													// Otherwise, blur the
													// active item since it is
													// no longer visible.
													if (this.active
															&& this.active
										...
										"mouseenter .ui-menu-item" : function(
											...
											// Remove ui-state-active class from
											// siblings of the newly focused
											// menu item
											// to avoid a jump caused by
											// adjacent elements both having a
											// class with a border
						...
						_destroy : function() {
							...
							this.element
									...
									.removeAttr("role").removeAttr("tabIndex")
									.removeAttr("aria-labelledby").removeAttr(
											"aria-expanded").removeAttr(
											"aria-hidden").removeAttr(
											"aria-disabled").removeUniqueId()
									.show();
						...
						_keydown : function(event) {
							...
							default:
								...
								match = this.activeMenu.children(
										".ui-menu-item").filter(
										function() {
											return regex.test($(this).children(
													"a").text());
										});
								match = skip
										&& match.index(this.active.next()) !== -1 ? this.active
										.nextAll(".ui-menu-item")
										: match;

								// If no matches on the current filter, reset to
								// the last character pressed
								// to move down the menu to the first item that
						...
						refresh : function() {
							var menus, icon = this.options.icons.submenu, submenus = this.element
							...
							menus.children(":not(.ui-menu-item):has(a)")
									.addClass("ui-menu-item").attr("role",
											"presentation").children("a")
									.uniqueId().addClass("ui-corner-all").attr(
											{
												tabIndex : -1,
												role : this._itemRole()
											});

							// Initialize unlinked menu-items containing spaces
							...
							// Add aria-disabled attribute to any disabled menu
							// item
							menus.children(".ui-state-disabled").attr(
									"aria-disabled", "true");

							// If the active item has been removed, blur the
							// menu
							if (this.active
									&& !$.contains(this.element[0],
											this.active[0])) {
						...
						_setOption : function(key, value) {
							if (key === "icons") {
								this.element.find(".ui-menu-icon").removeClass(
										this.options.icons.submenu).addClass(
										value.submenu);
							}
							this._super(key, value);
						},
						...
						focus : function(event, item) {
							...
							focused = this.active.children("a").addClass(
									"ui-state-focus");
							// Only update aria-activedescendant if there's a
							// role
						...
						_startOpening : function(submenu) {
							...
							// Don't open if already open fixes a Firefox bug
							// that caused a .5 pixel
							// shift in the submenu position when mousing over
							// the carat icon
						...
						_open : function(submenu) {
							...
							this.element.find(".ui-menu").not(
									submenu.parents(".ui-menu")).hide().attr(
									"aria-hidden", "true");

							submenu.show().removeAttr("aria-hidden").attr(
									"aria-expanded", "true").position(position);
						},

						collapseAll : function(event, all) {
							clearTimeout(this.timer);
							this.timer = this._delay(function() {
								// If we were passed an event, look for the
								// submenu that contains the event
								var currentMenu = all ? this.element : $(
										event && event.target).closest(
										this.element.find(".ui-menu"));

								// If we found no valid submenu ancestor, use
								// the main menu to close all sub menus anyway
						...
						// With no arguments, closes the currently active menu -
						// if nothing is active
						// it closes all menus. If passed an argument, it will
						// search for menus BELOW
						_close : function(startMenu) {
							...
							startMenu.find(".ui-menu").hide().attr(
									...
									"aria-expanded", "false").end().find(
									"a.ui-state-active").removeClass(
									"ui-state-active");
						},
						...
						expand : function(event) {
							...
							if (newItem && newItem.length) {
								...
								// activedescendant change in expanding submenu
								// from AT
								this._delay(function() {
									this.focus(event, newItem);
								});
						...
						_move : function(direction, filter, event) {
							...
							if (this.active) {
								if (direction === "first"
									...
									next = this.active[direction + "All"](
											".ui-menu-item").eq(0);
								}
							}
						...
						nextPage : function(event) {
							...
							if (this._hasScroll()) {
								...
								this.active.nextAll(".ui-menu-item").each(
										function() {
											item = $(this);
											return item.offset().top - base
													- height < 0;
										});
								...
								this
										.focus(
												event,
												this.activeMenu
														.children(".ui-menu-item")[!this.active ? "first"
														: "last"]());
							}
						},
						...
						previousPage : function(event) {
							...
							if (this._hasScroll()) {
								base = this.active.offset().top;
								height = this.element.height();
								this.active.prevAll(".ui-menu-item").each(
										function() {
											item = $(this);
											return item.offset().top - base
													+ height > 0;
								...
								this.focus(event, this.activeMenu.children(
										".ui-menu-item").first());
							}
						},
						...
						select : function(event) {
							// TODO: It should never be possible to not have an
							// active item at this
							// point, but the tests don't trigger mouseenter
							// before click.
					...
					"ui.progressbar",
					{
						...
						_create : function() {
							...
							this.element
									...
									.attr({
										// Only set static values, aria-valuenow
										// and aria-valuemax are
						...
						_constrainedValue : function(newValue) {
							...
							return this.indeterminate ? false : Math.min(
									this.options.max, Math.max(this.min,
											newValue));
						},

						_setOptions : function(options) {
							// Ensure "value" option is set after other values
							// (like max)
					...
					$.ui.mouse,
					{
						...
						_createHandles : function() {
							var i, handleCount, options = this.options, existingHandles = this.element
									.find(".ui-slider-handle").addClass(
											"ui-state-default ui-corner-all"), handle = "<a class='ui-slider-handle ui-state-default ui-corner-all' href='#'></a>", handles = [];
							...
							if (options.range) {
								...
								if (!this.range || !this.range.length) {
									...
									this.range
											...
											// Handle range switching from true
											// to min/max
											.css({
												"left" : "",
												"bottom" : ""
						...
						_setupEvents : function() {
							var elements = this.handles.add(this.range).filter(
									"a");
							this._off(elements);
						...
						_mouseCapture : function(event) {
							...
							this.handles
									.each(function(i) {
										var thisDistance = Math.abs(normValue
												- that.values(i));
						...
						_normValueFromMouse : function(position) {
							...
							valueMouse = this._valueMin() + percentMouse
									* valueTotal;
						...
						_trimAlignValue : function(val) {
							...
							var step = (this.options.step > 0) ? this.options.step
									: 1, valModStep = (val - this._valueMin())
					...
					"ui.spinner",
					{
						...
						_events : {
							...
							"mousedown .ui-spinner-button" : function(event) {
								...
								// We never want the buttons to have focus;
								// whenever the user is
								// interacting with the spinner, the focus
								// should be on the input.
								// If the input is focused then this.previous is
								// properly set from
								// when the input first received focus. If the
								// input is not focused
								// then we need to set this.previous based on
								// the value before spinning.
								...
								// and check (again) if focus moved off of the
								// input.
								this.cancelBlur = true;
								this._delay(function() {
									delete this.cancelBlur;
							...
							"mouseenter .ui-spinner-button" : function(event) {
								// button will add ui-state-active if mouse was
								// down while mouseleave and kept down
							...
							// shouldn't we just stop the repeater and wait
							// until mouseup before
						...
						_draw : function() {
							var uiSpinner = this.uiSpinner = this.element
									.addClass("ui-spinner-input").attr(
											"autocomplete", "off").wrap(
											this._uiSpinnerHtml()).parent()
									// add buttons
							...
							this.buttons = uiSpinner.find(".ui-spinner-button")
									.attr("tabIndex", -1).button().removeClass(
											"ui-corner-all");

							// IE 6 doesn't understand height: 50% for the
							// buttons
							// unless the wrapper has an explicit height
							if (this.buttons.height() > Math.ceil(uiSpinner
									.height() * 0.5)
									&& uiSpinner.height() > 0) {
						...
						_precision : function() {
							...
							if (this.options.min !== null) {
								precision = Math.max(precision, this
						...
						_precisionOf : function(num) {
							var str = num.toString(), decimal = str
									.indexOf(".");
							return decimal === -1 ? 0 : str.length - decimal
									- 1;
						},
						...
						_adjustValue : function(value) {
							...
							// - find out where we are relative to the base (min
							// or 0)
							base = options.min !== null ? options.min : 0;
							aboveMin = value - base;
							// - round to the nearest step
							aboveMin = Math.round(aboveMin / options.step)
									* options.step;
							// - rounding is based on 0, so adjust back to our
							// base
							value = base + aboveMin;
						...
						_format : function(value) {
							...
							return window.Globalize
									&& this.options.numberFormat ? Globalize
									.format(value, this.options.numberFormat,
						...
						_refresh : function() {
							this.element.attr({
								...
								"aria-valuemax" : this.options.max,
								// TODO: what should we do with values that
								// can't be parsed?
								"aria-valuenow" : this._parse(this.element
										.val())
							});
						},
						...
						_destroy : function() {
							this.element.removeClass("ui-spinner-input").prop(
									"disabled", false).removeAttr(
									"autocomplete").removeAttr("role")
									.removeAttr("aria-valuemin").removeAttr(
											"aria-valuemax").removeAttr(
											"aria-valuenow");
...
(function($, undefined) {
	...
	function isLocal(anchor) {
		return anchor.hash.length > 1
				&& decodeURIComponent(anchor.href.replace(rhash, "")) === decodeURIComponent(location.href
						.replace(rhash, ""));
	}
			...
			.widget(
					"ui.tabs",
					{
						...
						_create : function() {
							...
							this.element
									...
									.toggleClass("ui-tabs-collapsible",
											options.collapsible)
									// Prevent users from focusing disabled tabs
									// via click
									.delegate(
											".ui-tabs-nav > li",
									...
									// from focusing the element, so if the
									// anchor gets focused, blur.
							...
							if ($.isArray(options.disabled)) {
								options.disabled = $.unique(
										options.disabled.concat($.map(this.tabs
						...
						_tabKeydown : function(event) {
							/* jshint maxcomplexity:15 */
							var focusedTab = $(this.document[0].activeElement)
						...
						_findNextTab : function(index, goingForward) {
							...
							while ($
									.inArray(constrain(), this.options.disabled) !== -1) {
						...
						_tabId : function(tab) {
							return tab.attr("aria-controls") || "ui-tabs-"
									+ getNextTabId();
						},
						...
						refresh : function() {
							...
							if (options.active === false
								...
								if (this.tabs.length === options.disabled.length) {
									...
									this._activate(this._findNextTab(Math.max(
											0, options.active - 1), false));
								}
						...
						_setupDisabled : function(disabled) {
							...
							for ( var i = 0, li; (li = this.tabs[i]); i++) {
								if (disabled === true
									...
									$(li).addClass("ui-state-disabled").attr(
											"aria-disabled", "true");
								} else {
						...
						_setupHeightStyle : function(heightStyle) {
							...
							if (heightStyle === "fill") {
								...
								this.element.children().not(this.panels).each(
										function() {
						...
						_eventHandler : function(event) {
							...
							options.active = collapsing ? false : this.tabs
									.index(tab);
						...
						_toggle : function(event, eventData) {
							...
							function show() {
								...
								if (toShow.length && that.options.show) {
									that._show(toShow, that.options.show,
											complete);
								} else {
									toShow.show();
									complete();
						...
						enable : function(index) {
							...
							if (index === undefined) {
								...
								if ($.isArray(disabled)) {
									...
									disabled = $.map(this.tabs, function(li,
											num) {
										return num !== index ? num : null;
									});
								}
						...
						load : function(index, event) {
							...
							this.xhr = $.ajax(this._ajaxSettings(anchor, event,
									eventData));
...
(function($) {
	...
	function addDescribedBy(elem, id) {
		...
		describedby.push(id);
		elem.data("ui-tooltip-id", id).attr("aria-describedby",
				$.trim(describedby.join(" ")));
	}

	function removeDescribedBy(elem) {
		var id = elem.data("ui-tooltip-id"), describedby = (elem
				.attr("aria-describedby") || "").split(/\s+/), index = $
					...
					"ui.tooltip",
					{
						...
						options : {
							content : function() {
								...
								// .text() can't accept undefined, so coerce to
								// a string
								var title = $(this).attr("title") || "";
								// Escape title, since we're going from an
								// attribute to raw HTML
								return $("<a>").text(title).html();
							},
							hide : true,
							// Disabled elements have inconsistent behavior
							// across browsers (#8661)
						...
						_create : function() {
							...
							this.tooltips = {};
							// IDs of parent tooltips where we removed the title
							// attribute
							this.parents = {};
						...
						open : function(event) {
							...
							// No element to show a tooltip for or the tooltip
							// is already open
						...
						_updateContent : function(target, event) {
							...
							content = contentOption.call(target[0], function(
									response) {
								...
								// IE may instantly serve a cached response for
								// ajax requests
								// delay this call to _open so the other call to
								// _open runs first
								that._delay(function() {
									// jQuery creates a special event for
									// focusin when it doesn't
									// exist natively. To improve performance,
									// the native event
									// object is reused and the type is changed.
									// Therefore, we can't
									// rely on the type being correct after the
									// event finished
									// bubbling, so we set it back to the
									// previous value. (#8740)
									if (event) {
						...
						_open : function(event, target, content) {
							var tooltip, events, delayedShow, positionOption = $
							...
							// Content can be updated multiple times. If the
							// tooltip already
							...
							// if we have a title, clear it to prevent the
							// native tooltip
							// we have to check first to avoid defining a title
							// if none exists
							// (we don't want to cause an element to start
							// matching [title])
							//
							// We use removeAttr only for key events, to allow
							// IE to export the correct
							// accessible attributes. For mouse events, set to
							// empty string to avoid
							// native tooltip showing up (happens only when
							// removing inside mouseover).
							if (target.is("[title]")) {
								if (event && event.type === "mouseover") {
									target.attr("title", "");
								} else {
							...
							if (this.options.track && event
								...
								// trigger once to override element-relative
								// positioning
								position(event);
							} else {
							...
							// Handle tracking tooltips that are shown with a
							// delay (#8644). As soon
							// as the tooltip is visible, position the tooltip
						...
						close : function(event) {
							var that = this, target = $(event ? event.currentTarget
									: this.element), tooltip = this
									._find(target);

							// disabling closes the tooltip, so we need to track
							// when we're closing
							// to avoid an infinite loop in case the tooltip
							// becomes disabled on close
							if (this.closing) {
								return;
							}
							...
							// only set title if we had one before (see comment
							// in _open())
						...
						_tooltip : function(element) {
							...
							$("<div>").addClass("ui-tooltip-content").appendTo(
									tooltip);
				...
				.extend(
						color.prototype,
						{
							...
							toRgbaString : function() {
								var prefix = "rgba(", rgba = jQuery.map(
										this._rgba, function(v, i) {
											return v == null ? (i > 2 ? 1 : 0)
													: v;
										});
							...
							toHslaString : function() {
								var prefix = "hsla(", hsla = jQuery.map(this
										.hsla(), function(v, i) {
									if (v == null) {
										v = i > 2 ? 1 : 0;
									}
						...
						jQuery.cssHooks[hook] = {
							set : function(elem, value) {
								...
								try {
									...
									// errors on "invalid" values like 'auto' or
									// 'inherit'
								}
							}
						};
		...
		$.effects.animateClass = function(value, duration, easing, callback) {
			...
			return this
					.queue(function() {
						...
						// a promise
						allAnimations = allAnimations.map(function() {
							var styleInfo = this, dfd = $.Deferred(), opts = $
									.extend({}, o, {
										queue : false,
										complete : function() {
			...
			removeClass : (function(orig) {
				return function(classNames, speed, easing, callback) {
					return arguments.length > 1 ? $.effects.animateClass.call(
							this, {
								remove : classNames
							}, speed, easing, callback) : orig.apply(this,
							arguments);
				};
			})($.fn.removeClass),
		...
		$.fn
				.extend({
					effect : function( /* effect, options, speed, callback */) {
						...
						return queue === false ? this.each(run) : this.queue(
								queue || "fx", run);
					},
...
(function($, undefined) {
	...
	$.effects.effect.blind = function(o, done) {
		...
		var el = $(this), props = [ "position", "top", "bottom", "left",
				"right", "height", "width" ], mode = $.effects.setMode(el,
				o.mode || "hide"), direction = o.direction || "up", vertical = rvertical
...
(function($, undefined) {
	...
	$.effects.effect.bounce = function(o, done) {
		...
		if (show) {
			...
			el.css("opacity", 0)
					.css(ref, motion ? -distance * 2 : distance * 2).animate(
		...
		// "inprogress")
		if (queuelen > 1) {
			queue.splice.apply(queue, [ 1, 0 ].concat(queue.splice(queuelen,
					anims + 1)));
		}
		el.dequeue();
...
(function($, undefined) {
	...
	$.effects.effect.clip = function(o, done) {
		...
		var el = $(this), props = [ "position", "top", "bottom", "left",
				"right", "height", "width" ], mode = $.effects.setMode(el,
				o.mode || "hide"), show = mode === "show", direction = o.direction
...
(function($, undefined) {
	...
	$.effects.effect.drop = function(o, done) {
		...
		var el = $(this), props = [ "position", "top", "bottom", "left",
				"right", "opacity", "height", "width" ], mode = $.effects
		...
		distance = o.distance
				|| el[ref === "top" ? "outerHeight" : "outerWidth"](true) / 2;
...
(function($, undefined) {
	...
	$.effects.effect.explode = function(o, done) {
		...
		width = Math.ceil(el.outerWidth() / cells), height = Math.ceil(el
				.outerHeight()
				/ rows), pieces = [],
...
(function($, undefined) {
	...
	$.effects.effect.fold = function(o, done) {
		...
		var el = $(this), props = [ "position", "top", "bottom", "left",
				"right", "height", "width" ], mode = $.effects.setMode(el,
				o.mode || "hide"), show = mode === "show", hide = mode === "hide", size = o.size || 15, percent = /([0-9]+)%/
		...
		// Animate
		wrapper.animate(animation1, duration, o.easing).animate(animation2,
...
(function($, undefined) {
	...
	$.effects.effect.highlight = function(o, done) {
		var elem = $(this), props = [ "backgroundImage", "backgroundColor",
				"opacity" ], mode = $.effects.setMode(elem, o.mode || "show"), animation = {
...
(function($, undefined) {
	...
	$.effects.effect.scale = function(o, done) {
		...
		var el = $(this), options = $.extend(true, {}, o), mode = $.effects
				.setMode(el, o.mode || "effect"), percent = parseInt(o.percent,
				10)
				|| (parseInt(o.percent, 10) === 0 ? 0 : (mode === "hide" ? 0
						: 100)), direction = o.direction || "both", origin = o.origin, original = {
			height : el.height(),
			width : el.width(),
	...
	$.effects.effect.size = function(o, done) {
		...
		// Create element
		var original, baseline, factor, el = $(this), props0 = [ "position",
		...
		mode = $.effects.setMode(el, o.mode || "effect"), restore = o.restore
				...
				|| [ "middle", "center" ], position = el.css("position"), props = restore ? props0
				: props1, zero = {
			height : 0,
			width : 0,
			outerHeight : 0,
		...
		if (scale === "box" || scale === "both") {
			...
			if (factor.from.y !== factor.to.y) {
				...
				el.from = $.effects.setTransition(el, vProps, factor.from.y,
						el.from);
			...
			if (factor.from.x !== factor.to.x) {
				...
				el.from = $.effects.setTransition(el, hProps, factor.from.x,
						el.from);
		...
		if (scale === "content" || scale === "both") {
			...
			if (factor.from.y !== factor.to.y) {
				...
				el.from = $.effects.setTransition(el, cProps, factor.from.y,
						el.from);
		...
		if (scale === "content" || scale === "both") { // Scale the children
			...
			vProps = vProps.concat([ "marginTop", "marginBottom" ]).concat(
					cProps);
...
(function($, undefined) {
	...
	$.effects.effect.shake = function(o, done) {
		...
		var el = $(this), props = [ "position", "top", "bottom", "left",
				"right", "height", "width" ], mode = $.effects.setMode(el,
				o.mode || "effect"), direction = o.direction || "left", distance = o.distance || 20, times = o.times || 3, anims = times * 2 + 1, speed = Math
		...
		el.animate(animation1, speed, o.easing).animate(animation, speed / 2,
...
(function($, undefined) {
	...
	$.effects.effect.slide = function(o, done) {
		...
		// Create element
		var el = $(this), props = [ "position", "top", "bottom", "left",
				"right", "width", "height" ], mode = $.effects.setMode(el,
				o.mode || "show"), show = mode === "show", direction = o.direction
				|| "left", ref = (direction === "up" || direction === "down") ? "top"
		...
		el.show();
		distance = o.distance
				|| el[ref === "top" ? "outerHeight" : "outerWidth"](true);
